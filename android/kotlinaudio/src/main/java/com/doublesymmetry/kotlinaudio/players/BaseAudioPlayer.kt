...
import com.doublesymmetry.kotlinaudio.players.components.LocalFileExtractorsFactory
...
abstract class BaseAudioPlayer internal constructor(
    ...
    protected fun getMediaSourceFromAudioItem(audioItem: AudioItem): MediaSource {
        ...
        return when (audioItem.type) {
            ...
            else -> createProgressiveSource(mediaItem, factory, uri)
        }
    }
    ...
    private fun createProgressiveSource(
        ...
        // Podink: a file on disk gets sample-exact MP3 seeking (see
        // LocalFileExtractorsFactory); a stream keeps ExoPlayer's Xing/constant-
        // bitrate seeking, since index seeking would download everything between
        // the playhead and a far seek target.
        val extractorsFactory = if (isUriLocalFile(uri)) LocalFileExtractorsFactory()
        else DefaultExtractorsFactory().setConstantBitrateSeekingEnabled(true)
